...
export function InstrumentationProvider({
  ...
  // Dynamically adjust dotted background opacity based on scroll progress
  useEffect(() => {
    ...
    const update = () => {
      ...
      const progress = Math.min(1, Math.max(0, window.scrollY / maxScroll)); // 0 at top, 1 at bottom

      const styles = getComputedStyle(root);
      const min =
        parseFloat(styles.getPropertyValue("--dot-opacity-min")) || 0.08;
      const max =
        parseFloat(styles.getPropertyValue("--dot-opacity-max")) || 0.16;

      // Fade dots slightly as user scrolls down
      const value = (max - (max - min) * progress).toFixed(3);
      root.style.setProperty("--dot-opacity-current", String(value));
    };
  ...
  // Removed dotted background click interaction
  useEffect(() => {
    // no-op
  }, []);
